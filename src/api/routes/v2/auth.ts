...
authV2Router.post("/login", loginRateLimit(), async (req: AuthedRequest, res: Response) => {
  try {
    ...
    const user = sanitizeUser(userDoc, roles);
...
authV2Router.get("/me", ensureAuth, async (req: AuthedRequest, res: Response) => {
  try {
    ...
    const user = sanitizeUser(userDoc, req.auth?.roles || ["user"]);
...
authV2Router.put("/profile", ensureAuth, async (req: AuthedRequest, res: Response) => {
  try {
    ...
    const user = sanitizeUser(updatedUser, req.auth?.roles || ["user"]);
...
function sanitizeUser(userDoc: Record<string, any>, roles: string[]) {
  return {
    email: userDoc.email,
